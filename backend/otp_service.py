...
class OTPService:
    ...
    def __init__(self):
        if TWILIO_ACCOUNT_SID and TWILIO_AUTH_TOKEN:
            ...
            self.client = None
            print("⚠️ Twilio credentials not configured. OTP will be logged only.")
    ...
    def send_sms(self, phone_number: str, message: str) -> Dict[str, Any]:
        ...
        try:
            if not self.client or not TWILIO_PHONE_NUMBER:
                # For testing/development - log OTP instead of sending
    ...
    def create_otp_verification(
        ...
        with db_session() as db:
            # Clean up any existing pending OTPs for this phone/email
            existing = db.execute(
    ...
    def resend_otp(self, phone_number: str, email: str) -> Dict[str, Any]:
        ...
        with db_session() as db:
            # Find the most recent OTP verification
            otp_verification = db.execute(
                select(OtpVerification).where(
                    OtpVerification.phone_number == phone_number,
                    OtpVerification.email == email,
...
# Create global OTP service instance
